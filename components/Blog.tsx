    ...
    <div style={{ fontFamily: "'Inter',sans-serif", color: INK_TEXT, background: "#fff" }}>
      ...
      <section style={{ background: "#fff", padding: `clamp(3rem,6vw,5rem) ${PAD} clamp(4rem,8vw,7rem)` }}>
        <div ref={gridRef} className="blog-grid" style={{ maxWidth: MAXW, margin: "0 auto" }}>
          {posts.map((p, i) => (
            <PostCard key={p.slug} slug={p.slug} title={p.title} category={p.category} date={p.date} excerpt={p.excerpt} show={gridIn} index={i} />
          ))}
        </div>
      </section>
...
function PostCard({ slug, title, category, date, excerpt, show, index }: { slug: string; title: string; category: string; date: string; excerpt: string; show: boolean; index: number }) {
    ...
    <div style={rise}>
      <a href={`/blog/${slug}`} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
        ...
        <div style={{ aspectRatio: "3 / 2", overflow: "hidden", background: "#eaf1fc", flexShrink: 0 }}>
          ...
          <img src={img(category)} alt={`${title} — ${category} article from Vedvik Machinery`} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", transform: hover ? "scale(1.05)" : "scale(1)", transition: "transform 1.2s ease" }} />
        </div>
