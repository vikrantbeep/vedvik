...
const inputStyle: React.CSSProperties = {
  ...
  border: "1px solid rgba(2,0,98,.16)", fontFamily: "var(--font-inter, 'Inter'), sans-serif", fontSize: "0.95rem",
  color: INK_TEXT, background: "#fff", outline: "none",
};
const labelStyle: React.CSSProperties = {
  fontFamily: "var(--font-montserrat, 'Montserrat'), sans-serif", fontWeight: 600, fontSize: "0.72rem", letterSpacing: "0.08em",
...
function Detail({ label, value, href }: { label: string; value: string; href?: string }) {
  return (
    <div style={{ marginBottom: "1.6rem" }}>
      <div style={{ fontFamily: "var(--font-montserrat, 'Montserrat'), sans-serif", fontWeight: 600, fontSize: "0.68rem", letterSpacing: "0.16em", textTransform: "uppercase", color: SKY, marginBottom: "0.35rem" }}>{label}</div>
      {href ? (
        <a href={href} style={{ fontFamily: "var(--font-inter, 'Inter'), sans-serif", fontSize: "1rem", lineHeight: 1.55, color: "#fff", textDecoration: "none" }}>{value}</a>
      ) : (
        <div style={{ fontFamily: "var(--font-inter, 'Inter'), sans-serif", fontSize: "1rem", lineHeight: 1.55, color: "rgba(255,255,255,.92)" }}>{value}</div>
      )}
    </div>
  );
}
...
export default function Contact({ settings = defaultSettings }: { settings?: SiteSettings } = {}) {
  const loaded = usePageReady();
  const [sent, setSent] = useState(false);
  const [sending, setSending] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const mapsQuery = encodeURIComponent(settings.mapsQuery || `${settings.addressLine1}, ${settings.addressLine2}`);
    ...
    <div style={{ fontFamily: "var(--font-inter, 'Inter'), sans-serif", color: INK_TEXT, background: "#fff" }}>
      ...
      <section className="page-hero" style={{ position: "relative", overflow: "hidden", padding: `calc(clamp(3.5rem,8vw,5.5rem) + 68px) ${PAD} clamp(3rem,6vw,4rem)` }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: MAXW, margin: "0 auto" }}>
          <div style={{ ...reveal(0.05), fontFamily: "var(--font-montserrat, 'Montserrat'), sans-serif", fontWeight: 600, fontSize: "0.76rem", letterSpacing: "0.22em", textTransform: "uppercase", color: SKY, marginBottom: "1.4rem" }}>Contact</div>
          <TextEffect as="h1" per="word" preset="fade-in-blur" trigger={loaded} speedReveal={1.1} speedSegment={0.3}
            style={{ fontFamily: "var(--font-montserrat, 'Montserrat'), sans-serif", fontWeight: 300, fontSize: "clamp(2.4rem,7vw,5.4rem)", lineHeight: 1.02, letterSpacing: "-0.03em", margin: 0, color: "#fff" }}>Let&apos;s talk</TextEffect>
      ...
      <section style={{ background: "#fff", padding: `clamp(3rem,7vw,5rem) ${PAD} clamp(4rem,8vw,7rem)` }}>
        <div className="contact-grid" style={{ maxWidth: MAXW, margin: "0 auto" }}>
          ...
          <div style={{ ...reveal(0.1), background: "#fff", border: "1px solid rgba(2,0,98,.1)", borderRadius: "18px", padding: "clamp(1.6rem,3vw,2.4rem)", boxShadow: "0 1px 2px rgba(2,0,98,.04)" }}>
            {sent ? (
              <div style={{ padding: "2rem 0", textAlign: "center" }}>
                <div style={{ fontFamily: "var(--font-montserrat, 'Montserrat'), sans-serif", fontWeight: 500, fontSize: "1.4rem", color: NAVY, marginBottom: "0.6rem" }}>Thank you.</div>
                <p style={{ fontFamily: "var(--font-inter, 'Inter'), sans-serif", fontSize: "1rem", color: "rgba(14,23,80,.66)", margin: 0 }}>We&apos;ve received your enquiry and will be in touch shortly.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} action={FORM_ENDPOINT} method="POST">
                ...
                {error && (
                  <div role="alert" style={{ marginBottom: "1rem", padding: "0.85rem 1rem", borderRadius: "10px", background: "rgba(200,30,30,.07)", border: "1px solid rgba(200,30,30,.25)", fontFamily: "var(--font-inter, 'Inter'), sans-serif", fontSize: "0.9rem", lineHeight: 1.55, color: "#8a1f1f" }}>
                    {error}
                  </div>
                )}
          ...
          <aside style={{ ...reveal(0.2), background: BRAND_GRADIENT, borderRadius: "18px", padding: "clamp(1.8rem,3vw,2.4rem)", color: "#fff" }}>
            <div style={{ fontFamily: "var(--font-montserrat, 'Montserrat'), sans-serif", fontWeight: 500, fontSize: "1.3rem", marginBottom: "1.6rem" }}>Reach us directly</div>
            <Detail label="Email" value={settings.email} href={`mailto:${settings.email}`} />
            <Detail label="Phone" value={settings.phone} href={`tel:${settings.phoneLink}`} />
            <Detail label="Address" value={`${settings.addressLine1}, ${settings.addressLine2}`} />

            {/* Clicking anywhere on the map opens Google Maps with directions to the factory. */}
            <a href={mapsDirectionsUrl} target="_blank" rel="noopener noreferrer" aria-label="Get directions to Vedvik Machinery on Google Maps"
              ...
              />
              <span style={{ position: "absolute", left: "50%", bottom: "0.8rem", transform: "translateX(-50%)", whiteSpace: "nowrap", background: "rgba(2,0,98,.85)", color: "#fff", fontFamily: "var(--font-inter, 'Inter'), sans-serif", fontSize: "0.78rem", fontWeight: 600, padding: "0.45rem 1rem", borderRadius: "2rem" }}>
                Tap for directions →
              </span>
            </a>
          </aside>
