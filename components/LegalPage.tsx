    ...
    <div style={{ fontFamily: "'Inter',sans-serif", color: INK_TEXT, background: "#fff" }}>
      ...
      <section className="page-hero" style={{ position: "relative", overflow: "hidden", padding: `calc(clamp(3.5rem,8vw,5.5rem) + 68px) ${PAD} clamp(3rem,6vw,4rem)` }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: MAXW, margin: "0 auto" }}>
          <div style={{ ...reveal(0.05), fontFamily: "'Montserrat',sans-serif", fontWeight: 600, fontSize: "0.76rem", letterSpacing: "0.22em", textTransform: "uppercase", color: SKY, marginBottom: "1.4rem" }}>{eyebrow}</div>
          <TextEffect as="h1" per="word" preset="fade-in-blur" trigger={loaded} speedReveal={1.1} speedSegment={0.3}
            style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 300, fontSize: "clamp(2.4rem,7vw,5.4rem)", lineHeight: 1.02, letterSpacing: "-0.03em", margin: 0, color: "#fff" }}>{title}</TextEffect>
          <p style={{ ...reveal(0.28), fontFamily: "'Inter',sans-serif", fontSize: "clamp(1rem,1.6vw,1.2rem)", lineHeight: 1.65, color: "rgba(255,255,255,.85)", maxWidth: "44rem", marginTop: "1.6rem" }}>{intro}</p>
        </div>
      </section>
      ...
      <section style={{ background: "#fff", padding: `clamp(3rem,7vw,5rem) ${PAD} clamp(4rem,8vw,7rem)` }}>
        <article style={{ maxWidth: "820px", margin: "0 auto" }}>
          <p style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.88rem", color: "rgba(14,23,80,.55)", margin: "0 0 2.4rem", paddingBottom: "1.6rem", borderBottom: "1px solid rgba(2,0,98,.1)" }}>
            Last updated: {updated}
          </p>
          {sections.map((sec, i) => (
            <ScrollRise key={i}>
              <div style={{ marginBottom: "2.2rem" }}>
                <h2 style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 600, fontSize: "clamp(1.2rem,2.4vw,1.6rem)", letterSpacing: "-0.01em", color: NAVY, margin: "0 0 0.9rem" }}>{sec.heading}</h2>
                {sec.paragraphs.map((p, j) => (
                  <p key={j} style={{ fontFamily: "'Inter',sans-serif", fontSize: "1.02rem", lineHeight: 1.75, color: "rgba(14,23,80,.78)", margin: "0 0 1rem" }}>{p}</p>
                ))}
              </div>
            </ScrollRise>
          ))}
          <div style={{ marginTop: "2.5rem", paddingTop: "2rem", borderTop: "1px solid rgba(2,0,98,.1)" }}>
            <p style={{ fontFamily: "'Inter',sans-serif", fontSize: "1rem", lineHeight: 1.7, color: "rgba(14,23,80,.78)", margin: 0 }}>
